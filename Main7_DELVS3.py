...
model_folder_VGG16_NCNN = 'Action_VGG16_NCNN_Model'
model_folder_VGG19_NCNN = 'Action_VGG19_NCNN_Model'
model_folder_ResNet50_NCNN = 'Action_ResNet50_NCNN_Model'
...
# checkpoint
checkpoint_dir_VGG16_NCNN = os.path.join(os.getcwd(), model_folder_VGG16_NCNN, 'checkpoints')
# load the best model
# If the error appears 'Error in loading the saved optimizer', it doesn't matter!
best_model_VGG16_NCNN = load_model(os.path.join(checkpoint_dir_VGG16_NCNN, "weights-best.hdf5"))
# checkpoint
checkpoint_dir_VGG19_NCNN = os.path.join(os.getcwd(), model_folder_VGG19_NCNN, 'checkpoints')
# load the best model
# If the error appears 'Error in loading the saved optimizer', it doesn't matter!
best_model_VGG19_NCNN = load_model(os.path.join(checkpoint_dir_VGG19_NCNN, "weights-best.hdf5"))
# checkpoint
checkpoint_dir_ResNet50_NCNN = os.path.join(os.getcwd(), model_folder_ResNet50_NCNN, 'checkpoints')
# load the best model
# If the error appears 'Error in loading the saved optimizer', it doesn't matter!
best_model_ResNet50_NCNN= load_model(os.path.join(checkpoint_dir_ResNet50_NCNN, "weights-best.hdf5"))
...
pred_prob_VGG16_NCNN = best_model_VGG16_NCNN.predict(test_features)
pred_prob_VGG19_NCNN = best_model_VGG19_NCNN.predict(test_features)
pred_prob_ResNet50_NCNN = best_model_ResNet50_NCNN.predict(test_features)
...
pred_labels_VGG16_NCNN = np.argmax(pred_prob_VGG16_NCNN, axis=1)
pred_labels_VGG19_NCNN = np.argmax(pred_prob_VGG19_NCNN, axis=1)
pred_labels_ResNet50_NCNN= np.argmax(pred_prob_ResNet50_NCNN, axis=1)
...
for i in range(0,len(test_features)):
    list_each = list([pred_labels_VGG16[i], pred_labels_VGG19[i], pred_labels_ResNet50[i],
                      pred_labels_VGG16_NCNN[i], pred_labels_VGG19_NCNN[i], pred_labels_ResNet50_NCNN[i]])
    counter_each_classes = np.array([list_each.count(0), list_each.count(1), list_each.count(2), list_each.count(3),
                                     list_each.count(4), list_each.count(5), list_each.count(6)])
...
# Averaging voting
pred_prob_avg = (pred_prob_VGG16 + pred_prob_VGG19 + pred_prob_ResNet50 + pred_prob_VGG16_NCNN + pred_prob_VGG19_NCNN + pred_prob_ResNet50_NCNN)/3
...
import pandas as pd
np.random.seed(123)
df = pd.DataFrame(columns=('w1', 'w2', 'w3','w4', 'w5', 'w6',  'acc'))
i = 0
for w1 in range(1,6):
    for w2 in range(1,6):
        for w3 in range(1,6):
                ...
                for w5 in range(1, 6):
                    for w6 in range(1, 6):
                        if len(set((w1, w2, w3, w4, w5, w6))) == 1:  # skip if all weights are equal
                            continue
                        pred_prob_weighted = (pred_prob_VGG16 * w1 + pred_prob_VGG19 * w2 + pred_prob_ResNet50 * w3 +
                                              pred_prob_VGG16_NCNN * w4 + pred_prob_VGG19_NCNN * w5 + pred_prob_ResNet50_NCNN * w6) / \
                                             (w1 + w2 + w3 + w4 + w5 + w6)
                        ...
                        df.loc[i] = [w1, w2, w3, w4, w5, w6, acc]
                        i += 1

df_sort = df.sort_values(by=['acc'], ascending=False)

# Averaging voting
w1_new = df_sort.iloc[0,0]
w2_new = df_sort.iloc[0,1]
w3_new = df_sort.iloc[0,2]
w4_new = df_sort.iloc[0,3]
w5_new = df_sort.iloc[0,4]
w6_new = df_sort.iloc[0,5]
pred_prob_weighted = (pred_prob_VGG16*w1_new  + pred_prob_VGG19*w2_new  + pred_prob_ResNet50*w3_new +
                      pred_prob_VGG16_NCNN*w4_new  + pred_prob_VGG19_NCNN*w5_new  + pred_prob_ResNet50_NCNN*w6_new)/(w1_new+w2_new+w3_new+w4_new+w5_new+w6_new)
# pred_prob_mean = pred_prob_VGG16*0.4 + pred_prob_VGG19*0.3 + pred_prob_ResNet50*0.3
# The index for prediction of testing set
pred_labels_weighted = np.argmax(pred_prob_weighted, axis=1)
# The scores for prediction of testing set
